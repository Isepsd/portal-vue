...
const form = ref({
  ...
  kinerja_scada: 'SCADA', // Default value
  nama_induk_pointtype: 'RTU',
  nama_pointtype: '',
});

const idPointWatch = ref(form.value.nama_induk_pointtype);

watch(() => form.value.nama_induk_pointtype, (newValue) => {
  idPointWatch.value = newValue;
  console.log('Nama Induk Point Type changed to:', newValue);
});

// Watch untuk memantau perubahan kinerja_scada
watch(() => form.value.kinerja_scada, (newValue) => {
  if (newValue !== 'SCADA') {
    // Reset atau perbarui logika sesuai dengan kondisi yang diinginkan
    form.value.nama_induk_pointtype = ''; // Misalnya reset nama_induk_pointtype jika kinerja_scada bukan 'SCADA'
  }
});
...
<template>
  <VRow class="match-height">
    <!-- Dynamic Async Select for Path 1 -->
 <VCol cols="6" md="3" sm="6" lg="2">
  ...
  />
    ...
    <!-- Static Select for Jenis Kinerja -->
    <VCol cols="6" md="3" sm="6" lg="2">
      <AppAutocomplete
        v-model="form.kinerja_scada"
        label="Jenis Kinerja"
        :items="['SCADA', 'RC', 'TRIP']"
        placeholder="Pilih Jenis Kinerja"
      />
    </VCol>

    <!-- Conditional Select for Tipe Point when Jenis Kinerja is SCADA -->
    <VCol v-if="form.kinerja_scada === 'SCADA'" cols="6" md="3" sm="6" lg="2">
      <AppAutocomplete
        v-model="form.nama_induk_pointtype"
    ...
    <VCol v-if="form.kinerja_scada === 'SCADA'" cols="6" md="3" sm="6" lg="2">
      <AppAutocomplete
        ...
        :queryParams="{
          ...
          induk: 0,
          sort_by: '-name',
          nama_induk_pointtype: idPointWatch // Menambahkan idPointWatch ke queryParams
        }"
      />
    </VCol>

    <!-- Dynamic Async Select for Path 2 (Only shown for RC or TRIP) -->
    <VCol v-if="form.kinerja_scada === 'RC' || form.kinerja_scada === 'TRIP'" cols="6" md="3" sm="6" lg="2">
      <AppAutocomplete
        v-model="form.path2"
        fieldName="path2"
