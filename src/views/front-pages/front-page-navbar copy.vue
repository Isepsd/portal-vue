...
const isPageActive = computed(() =>
  menuItems.some(item =>
    item.children.some(child => isCurrentRoute(child.to))
  )
)
            ...
            <div v-for="(item, index) in menuItems" :key="index">
              ...
              <ul class="mb-6">
                ...
                >
                  <RouterLink
                    :to="child.to"
                    :target="item.title === 'Page' ? '_self' : '_blank'"
                    class="mega-menu-item"
                    :class="isCurrentRoute(child.to) ? 'active-link' : 'text-high-emphasis'"
                  >
                  ...
                  <div class="nav-menu">
                    <div v-for="(item, index) in menuItems" :key="index">
                      <div class="d-flex align-center gap-x-3 mb-6">
                        <VAvatar variant="tonal" color="primary" rounded :icon="item.icon" />
                        <div class="text-body-1 text-high-emphasis font-weight-medium">
                          {{ item.title }}
                        </div>
                      </div>
                      <ul>
                        <li v-for="child in item.children" :key="child.name" style="list-style: none;" class="text-body-1 mb-4 text-no-wrap">
                          <RouterLink
                            class="mega-menu-item"
                            :to="child.to"
                            :target="item.title === 'Page' ? '_self' : '_blank'"
