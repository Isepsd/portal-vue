...
const filterValues = ref<any>({
  ...
  sort_by: '-tgl_update,id_ref_jenis_pembangkit',
  page: 1,
  limit: 10,
})
...
const handleRespDataApi = (data: any[], page: number, limit: number) =>
  data.map((item: any, idx: number) => {
   ...
   id_ref_jenis_pembangkit: item?.id_ref_jenis_pembangkit,
        id: item?.id_ref_jenis_pembangkit,
        nama: item?.nama,
    }
  })
...
const handleEdit = (item: any) => {
  router.push(`/jaringan/jenispembangkit/edit/${item.id_ref_jenis_pembangkit}`)
}

const handleAdd = () => {
  router.push('/jaringan/jenispembangkit/tambah')
}
 ...
 <!-- Header -->
  <VCard class="header-card mb-6 pa-4">
    <VRow class="align-center">
      ...
      <VCol cols="12" md="8" class="d-flex align-start">
        ...
        <div class="header-info">
          <h2 class="header-title">Jenis Pembangkit</h2>
    ...
    <TableIgrid
      ...
      :pathService="API_PATH().master.jaringan.jenis_pembangkit"
