...
import { HISTORI_SOE_COLUMNS } from '../component/columnref/HistoriColumn.config'
...
const columnDefs = ref(HISTORI_SOE_COLUMNS())
...
const handleRespDataApi = (data: any[], page: number, limit: number) =>
  data.map((item: any, idx: number) => {
    return {
        ...
        time_stamp: item?.msec ,
        tanggal: item?.system_time_stamp,
...
<VCol cols="12">
  <AppCardCode title="Filter" :code="{ ts: '', js: '' }">
    <div class="mt-6">
      <FillterFasopMessagehistory :onFilterChange="handleFilterChange" />
    </div>
  </AppCardCode>
</VCol>
    ...
    <TableIgrid
      ...
      :pathService="API_PATH().fasop.laporan_scada.soe_alarm_proteksi"
