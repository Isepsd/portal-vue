...
const handleFormSubmit = (result: any) => {
  if (result.success) {
    ...
    showNotification(`Sukses ${action} data Fasilitas`, 'success')
  } else {
...
// Initialize
onMounted(async () => {
  console.log('Fasilitas form mounted')
})
</script>
...
<template>
  ...
  <FormData
    ...
    :path="API_PATH().master.opsisdis.rekap_padam.fasilitas"
    custom-label="Fasilitas"
