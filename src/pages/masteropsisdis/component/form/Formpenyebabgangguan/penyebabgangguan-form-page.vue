...
const handleFormSubmit = (result: any) => {
  if (result.success) {
    ...
    showNotification(`Sukses ${action} data Penyebab Gangguan`, 'success')
  } else {
...
// Initialize
onMounted(async () => {
  console.log('Penyebab Gangguan form mounted')
})
</script>
...
<template>
  ...
  <FormData
    ...
    :path="API_PATH().master.opsisdis.rekap_padam.penyebab"
    custom-label="Penyebab Gangguan"
  ...
  >
    <template #default="{ form, loading, errors, fetchDynamicData, validateForm, submitForm }">
      <VRow class="match-height">
        <!-- Nama Penyebab Gangguan -->
        <VCol cols="12" md="7">
          <VTextField
            v-model="form.nama"
            label="Nama Penyebab Gangguan *"
            placeholder="Masukkan Nama Penyebab Gangguan"
