...
interface Props {
  ...
  isTree?: boolean
  treeData?: any[]
  onTreeRowClicked?: (event: any) => void
  ...
  try {
    ...
    if (req?.data) {
      ...
      if (Array.isArray(req.data)) {
        ...
        results = req.data
        total = req.data.length
      } else if (req.data.results) {
        // Nested results
        results = req.data.results
        total = req.data.total || req.data.results.length
      } else {
        // Object with data
        results = Object.values(req.data)
        total = results.length
      }
    } else if (req?.results) {
      // Direct results property
      results = req.results
      total = req.total || req.results.length
...
// 🌳 For tree data, use treeData prop directly
const treeRowData = computed(() => {
  if (props.isTree && props.treeData.length > 0) {
...
const columnDefs = computed(() => {
  ...
  // For tree mode, don't add No and Action columns
  if (!props.isTree) {
    // Kolom No
    columns.push({
      headerName: 'No',
...
onMounted(() => {
  // Only load data from API if not in tree mode
  if (!props.isTree) {
    getData(internalFilterValues.value.page, internalFilterValues.value.limit)
  }
})
...
<AgGridVue
  ...
  :gridOptions="{
    ...
    onRowClicked: props.isTree ? props.onTreeRowClicked : undefined,
    autoGroupColumnDef: props.isTree ? {
    ...
    getDataPath: props.isTree ? (data: any) => {
      return data.path || [data.nama_pointtype]
    } : undefined,
  }"
  :sideBar="false"
  @filterChanged="!props.isTree ? onFilterChanged : undefined"
  @cellClicked="!props.isTree ? onCellClicked : undefined"
/>
